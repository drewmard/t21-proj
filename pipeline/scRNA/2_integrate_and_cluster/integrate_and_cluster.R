...
f.out <- paste0(dir,"/output/data/",DATASET,"/RNA_FindNeighbors.rds")
...
if (start == 2) {
  f <- paste0(dir,"/output/data/",DATASET,"/RNA_FindNeighbors.rds")
...
if (end > 1 & start <= 2) {
  ...
  # Save data:
  f.out <- paste0(dir,"/output/data/",DATASET,"/RNA_FindClusters.rds")
  print(paste0("Saving data: ",f.out,"..."))
  saveRDS(dfcombined,file = f.out)
  
  print("Creating UMAP with seurat clusters...")
  # p1 <- DimPlot(dfcombined, group.by = "seurat_clusters")
  f.out <- paste0(dir,"/output/data/",DATASET,"/RNA_umap.seurat_clusters.png")
