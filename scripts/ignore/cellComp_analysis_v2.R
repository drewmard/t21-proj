...
for (sampletype in c("Femur","Liver")) { 
  ...
  for (disease_status in c("DownSyndrome","Healthy")) {
    ...
    all_patients[[sampletype]][[disease_status]] <- unique(df[[sampletype]][[disease_status]]$patient)
...
for (sorting_strategy in c("CD45+","CD235a-")) {
  ...
  for (cell_type in cell_type_groups) {
    ...
    iter=0; for (clustertype in cluster.uniq) {
      subset(mat1,cluster==clustertype)
    }
    ...
    all_patients[!(all_patients %in% cluster_percentages_all[[sampletype]][[sorting_strategy]][[cell_type]]$Patient_ID)]
    ...
    c(all_patients[[sampletype]][["Healthy"]],all_patients[[sampletype]][["DownSyndrome"]])
