...
# dataf$combi_clust = dataf$combi_annot_v3
...
# colnames(new_annot_mapping)[1] = "combi_clust"
# colnames(dataf)[colnames(dataf)=="organ"] <- "Organ"
# colnames(dataf)[colnames(dataf)=="disease_status"] <- "DiseaseStatus"

# dataf = fread("/Volumes/Seagate Portable Drive/data/a3_tab.obs.withannot.csv",data.table = F,stringsAsFactors = F)
head(dataf)
sum(duplicated(df$barcodekey))

tab=table(df$environment,df$combi_annot)
tab/rowSums(tab)
...
# new_annot_mapping = fread("~/Downloads/new_annot_mapping.csv",data.table = F,stringsAsFactors = F)
dataf3 = merge(dataf3,new_annot_mapping,by="combi_clust")
dataf3.old = dataf3
rm(dataf2)
rm(df)
rm(dataf)
...
tab = aggregate(dataf3$combi_clust,by=list(annot=dataf3$broad_annot,organ=dataf3$Organ,disease=dataf3$DiseaseStatus),length)
library(dplyr)
tab <- tab %>%
...
tab2 = aggregate(dataf3$combi_clust,by=list(annot=dataf3$cell_type_groups,organ=dataf3$Organ,disease=dataf3$DiseaseStatus),length)
library(dplyr)
tab2 <- tab2 %>%
  group_by(organ, disease) %>%
  mutate(prop = x / sum(x))
...
ggplot(tab.mg,aes(x=prop.sep,y=prop.int,col=annot,shape=organ_disease)) + #,fill=disease)) + 
  geom_abline(slope=1,intercept=0,lty='dashed') + 
  geom_point(size=rel(3)) +
  scale_shape_manual(values = c(1,16,2,17)) +
  scale_fill_manual(values = c("white", "black")) +
  theme_bw() + theme(panel.grid = element_blank()) +
  labs(x="Separate",y="Integrated") +
  scale_color_brewer(palette="Set2")
  
tab = aggregate(dataf3$combi_clust,by=list(annot=dataf3$broad_annot,organ=dataf3$Organ,disease=dataf3$DiseaseStatus,sample=dataf3$patient_sample),length)
library(dplyr)
tab <- tab %>%
...
dataf2[dataf2$combi_clust%in%narrow_labels] = broad_label
