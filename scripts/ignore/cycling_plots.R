...
for (k in 1:length(genes_lst)) {
  f.out <- paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/cycling_plots/Violin.",genes_lst[k],".pdf")
...
# genes_lst = c("IGLL1", "IGHM", "CD79A")
for (k in 1:length(genes_lst)) {
  f.out <- paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/cycling_plots/",genes_lst[k],".violin.pdf")
  print(paste0(k,": ",f.out))
  pdf(f.out,width=17,height=7)
  print(VlnPlot(dfcombined,features=genes_lst[k],sort=FALSE,slot="data") + NoLegend() + geom_boxplot(width=0.1, fill="white"))
  dev.off()
  
  f.out <- paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/cycling_plots/",genes_lst[k],".ridge.pdf")
  print(paste0(k,": ",f.out))
  pdf(f.out,width=17,height=7)
