...
for (disease_status in c("Healthy","DownSyndrome")) {
  ...
  iter=0; for (cell_type in clusters_for_DE) {
    ...
    x$environment <- factor(x$environment,levels=c("Femur","Liver"))
    ...
    f.out <- paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/DE_pb_leiden_names/",disease_status,".",cell_type_filename,".",subset_column,".txt")
