...
library(ggplot2)
g1=ggplot(tab.mg,aes(x=prop.sep,y=prop.pulpescu,col=annot)) + #,fill=disease)) + 
  geom_abline(slope=1,intercept=0,lty='dashed') + 
  geom_point(size=rel(3)) +
  # scale_shape_manual(values = c(1,16,2,17),label=c("Ts21","Disomic")) +
  scale_fill_manual(values = c("white", "black")) +
...
cor.test(tab.mg$prop.sep,tab.mg$prop.t21)
cor.test(tab.mg$prop.sep,tab.mg$prop.pulpescu)
cor.test(tab.mg$prop.t21,tab.mg$prop.pulpescu)
