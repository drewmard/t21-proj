...
# in this step, we are going to downsample the liver data to the femur dataset size, for working with that dataset
...
for (permNum in 1:100) {
  ...
  for (patient_of_interest1 in patient_in_both) {
    ...
    if (length(potential_patients) > 1) {
    ...
    j = 0; identified=FALSE; while (j <= length(potential_patients) & !identified) {
      ...
      if (mean(summary1.sub$x < tmp$x[1:nrow(summary1.sub)])==1) {
        ...
        # print(head(new_summary1.nsamp,1))
        new_summary1.nsamp <- new_summary1.nsamp[!(new_summary1.nsamp$patient %in% res[[iter]]$patient),]
        # print(head(new_summary1.nsamp,1))
        identified=TRUE
      }
    }
  ...
  for (patient_of_interest1 in patient_in_both) {
    ...
    j = 0; identified=FALSE; while (j <= length(potential_patients) & !identified) {
      ...
      if (mean(summary2.sub$x < tmp$x[1:nrow(summary2.sub)])==1) {
        ...
        new_summary2.nsamp <- new_summary2.nsamp[!(new_summary2.nsamp$patient %in% res[[iter]]$patient),]
        identified=TRUE
      }
    }
  }
