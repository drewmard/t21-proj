...
f.out=paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/DE_liver_v_femur/",cell_type_filename,'.',"Healthy",'.FE.keep_t21_genes.txt')
df <- fread(f.out,data.table=F,stringsAsFactors = F)
df1.full <- df[!duplicated(df$names),]

f.out=paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/DE_liver_v_femur/",cell_type_filename,'.',"Healthy",'.FE.txt')
...
colnames(df1.full.lfc)[3] <- paste0(colnames(df1.full.lfc)[3],".h")
colnames(df1.full.p)[3] <- paste0(colnames(df1.full.p)[3],".h")
colnames(df1.full.fdr)[3] <- paste0(colnames(df1.full.fdr)[3],".h")
...
colnames(df1.full.lfc.tmp)[2] <- paste0(colnames(df1.full.lfc.tmp)[2],".t21")
colnames(df1.full.p.tmp)[2] <- paste0(colnames(df1.full.p.tmp)[2],".t21")
colnames(df1.full.fdr.tmp)[2] <- paste0(colnames(df1.full.fdr.tmp)[2],".t21")
...
for (permNum in 1:100) {
  ...
  f <- paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/DE_pb_leiden_names_downsamp/","DownSyndrome",".",cell_type_filename,".",subset_column,".perm",permNum,".txt")
...
# df1.full.fdr[order(df1.full.fdr$adj.P.Val.t21),][1,]
# df1.full.p[order(df1.full.p$P.Value.t21),][1,]
# df1.full.lfc[order(df1.full.p$P.Value.t21),][1,]
# 
...
permNum="ALL"
f.out <- paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/DE_pb_leiden_names_downsamp/","DownSyndrome",".",cell_type_filename,".",subset_column,".perm",permNum,".lfc.txt")
fwrite(df1.full.lfc,f.out,quote = F,na = "NA",sep = '\t',row.names = F,col.names = T)
f.out <- paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/DE_pb_leiden_names_downsamp/","DownSyndrome",".",cell_type_filename,".",subset_column,".perm",permNum,".p.txt")
fwrite(df1.full.p,f.out,quote = F,na = "NA",sep = '\t',row.names = F,col.names = T)
f.out <- paste0("/oak/stanford/groups/smontgom/amarder/t21-proj/out/full/DE_pb_leiden_names_downsamp/","DownSyndrome",".",cell_type_filename,".",subset_column,".perm",permNum,".fdr.txt")
